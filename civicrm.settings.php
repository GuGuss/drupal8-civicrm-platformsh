...
/*
 * Get the database informations from Platform.sh variables.
 */
$relationships = json_decode(base64_decode($_ENV['PLATFORM_RELATIONSHIPS']), TRUE);
$endpoint = $relationships['database'][0];

if (!defined('CIVICRM_UF_DSN')) {
  define( 'CIVICRM_UF_DSN', $endpoint['scheme'].'://'.$endpoint['username']."@".$endpoint['host'].'/'.$endpoint['path'].'?new_link=true' );
}
...
if (!defined('CIVICRM_DSN')) {
  define( 'CIVICRM_DSN', $endpoint['scheme'].'://'.$endpoint['username']."@".$endpoint['host'].'/'.$endpoint['path'].'?new_link=true' );
}
